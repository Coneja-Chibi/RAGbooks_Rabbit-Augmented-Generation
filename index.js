...
function getRAGSettings() {
    ...
    return {
        ...
        threshold: ragState.threshold ?? 0.5, // BUG FIX: Lowered from 0.80 to 0.65 for better recall
        scoreThreshold: ragState.scoreThreshold ?? 0.5,
...
const STOP_WORDS = new Set([
    ...
    // --- РАСШИРЕННЫЙ СПИСОК РУССКИХ СТОП-СЛОВ (ИСПРАВЛЕННЫЙ) ---
    // Предлоги
    'в', 'во', 'на', 'с', 'со', 'к', 'ко', 'у', 'за', 'по', 'от', 'из', 'до', 'для', 'без', 'под', 'над',
    'при', 'через', 'о', 'об', 'обо', 'перед', 'между', 'около', 'после', 'кроме', 'про', 'ради', 'среди',
    'против', 'внутри', 'вне', 'вместо', 'возле', 'вокруг', 'впереди', 'мимо', 'напротив', 'насчет',
    'поверх', 'посреди', 'сверх', 'сквозь', 'согласно',

    // Союзы
    'и', 'а', 'но', 'да', 'или', 'либо', 'если', 'что', 'чтобы', 'как', 'когда', 'пока', 'потому', 'поэтому',
    'так', 'также', 'тоже', 'зато', 'однако', 'же', 'ибо', 'хотя', 'будто', 'словно', 'точно', 'едва',
    'лишь', 'только', 'чем', 'нежели', 'причем', 'притом',

    // Местоимения (все падежные формы)
    ...
    'мой', 'моя', 'мое', 'моё', 'мои', 'моего', 'моей', 'моих', 'моему', 'моим', 'моими',
    'твой', 'твоя', 'твое', 'твоё', 'твои', 'твоего', 'твоей', 'твоих', 'твоему', 'твоим', 'твоими',
    'свой', 'своя', 'свое', 'своё', 'свои', 'своего', 'своей', 'своих', 'своему', 'своим', 'своими',
    'наш', 'наша', 'наше', 'наши', 'нашего', 'нашей', 'наших', 'нашему', 'нашим', 'нашими',
    'ваш', 'ваша', 'ваше', 'ваши', 'вашего', 'вашей', 'ваших', 'вашему', 'вашим', 'вашими',

    // Указательные местоимения
    'этот', 'эта', 'это', 'эти', 'этого', 'этой', 'этих', 'этому', 'этим', 'этими', 'этом',
    'тот', 'та', 'то', 'те', 'того', 'той', 'тех', 'тому', 'тем', 'теми', 'том',
    'такой', 'такая', 'такое', 'такие', 'такого', 'таких', 'такому', 'таким', 'такими',
    'сей', 'сия', 'сие', 'сии',

    // Вопросительные и относительные местоимения
    'кто', 'кого', 'чего', 'кому', 'чему', 'кем', 'чем', 'ком',
    'какой', 'какого', 'каких', 'какому', 'какими',
    'который', 'которая', 'которое', 'которые', 'которого', 'которой', 'которых', 'которому', 'которым', 'которыми',
    'чей', 'чья', 'чье', 'чьё', 'чьи', 'чьего', 'чьей', 'чьих', 'чьему', 'чьим', 'чьими',
    'сколько', 'столько', 'несколько',

    // Неопределенные местоимения
    'некто', 'нечто', 'некоторый', 'некий', 'какой-то', 'кто-то', 'что-то', 'кто-нибудь', 'что-нибудь',
    'какой-нибудь', 'кто-либо', 'что-либо', 'какой-либо', 'кое-кто', 'кое-что', 'кое-какой',

    // Отрицательные местоимения
    'никто', 'ничто', 'никого', 'ничего', 'никому', 'ничему', 'никем', 'ничем', 'никакой',
    'ничей', 'нигде', 'никуда', 'ниоткуда', 'никогда', 'никак',

    // Определительные местоимения
    'весь', 'вся', 'все', 'всё', 'всего', 'всей', 'всех', 'всему', 'всем', 'всеми', 'всём',
    'сам', 'сама', 'само', 'сами', 'самого', 'самой', 'самих', 'самому', 'самим', 'самими',
    'самый', 'самая', 'самое', 'самые', 'самых', 'самым', 'самыми',
    'каждый', 'каждая', 'каждое', 'каждые', 'любой', 'любая', 'любое', 'любые',
    'другой', 'другая', 'другое', 'другие', 'иной', 'иная', 'иное', 'иные',

    // Частицы
    'не', 'ни', 'нет', 'ли', 'бы', 'б', 'ну', 'вот', 'вон', 'ведь', 'даже', 'уж', 'уже',
    'еще', 'ещё', 'лишь', 'почти', 'разве', 'неужели', 'именно', 'раз', 'все-таки',
    'вряд', 'пусть', 'пускай', 'давай', 'давайте', 'дай', '-ка', 'просто', 'прямо',

    // Вспомогательные глаголы (разные формы)
    'быть', 'есть', 'был', 'была', 'было', 'были', 'буду', 'будешь', 'будет', 'будем', 'будете', 'будут',
    'бывший', 'бывшая', 'бывшее', 'бывшие', 'будучи', 'быв', 'бывало', 'бывает',
    'стать', 'стал', 'стала', 'стало', 'стали', 'стану', 'станешь', 'станет', 'станем', 'станете', 'станут',
    'становиться', 'становится', 'становился', 'становилась', 'становилось', 'становились',
    'мочь', 'могу', 'можешь', 'может', 'можем', 'можете', 'могут', 'мог', 'могла', 'могло', 'могли',
    'можно', 'нельзя', 'надо', 'нужно', 'необходимо', 'следует',

    // Наречия общего характера
    'очень', 'много', 'мало', 'больше', 'меньше', 'более', 'менее', 'весьма', 'слишком',
    'здесь', 'тут', 'там', 'туда', 'сюда', 'оттуда', 'отсюда', 'везде', 'всюду', 'где', 'куда', 'откуда',
    'тогда', 'теперь', 'сейчас', 'всегда', 'иногда', 'опять', 'снова', 'потом', 'затем',
    'насколько', 'настолько', 'почему', 'зачем', 'отчего',
    'хорошо', 'плохо', 'лучше', 'хуже', 'совсем', 'вообще', 'вполне', 'довольно',

    // Вводные слова
    'конечно', 'разумеется', 'безусловно', 'несомненно', 'возможно', 'вероятно', 'наверное',
    'кажется', 'видимо', 'по-видимому', 'очевидно', 'наконец', 'итак', 'следовательно', 'значит',
    'впрочем', 'кстати', 'например', 'скажем', 'допустим', 'положим', 'вообще-то',

    // Числительные (основные)
    'один', 'одна', 'одно', 'одни', 'два', 'две', 'три', 'четыре', 'пять', 'шесть', 'семь', 'восемь',
    'девять', 'десять', 'сто', 'тысяча', 'миллион', 'миллиард',
    'первый', 'второй', 'третий', 'последний',

    // Сокращения и разговорные формы
    'т.е.', 'т.д.', 'т.п.', 'др.', 'пр.', 'см.', 'напр.', 'тыс.', 'млн', 'млрд',
    'ага', 'ого', 'ох', 'ах', 'эх', 'увы', 'фу', 'ха', 'хм', 'гм', 'тьфу', 'брр', 'ой',

    // Дополнительные служебные слова
    'пожалуйста', 'спасибо', 'извините', 'простите', 'здравствуйте', 'до свидания', 'привет', 'пока',
    'часто', 'чаще', 'раздел', 'казалось', 'рядом', 'легко', 'видел', 'видела', 'видели', 'слышал',
	'слышала', 'слышали', 'чувствовал', 'чувствовала', 'сказала', 'сказал', 'смотреть', 'думал', 'думала'
]);
