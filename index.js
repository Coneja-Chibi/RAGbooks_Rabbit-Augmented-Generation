...
function getRAGSettings() {
    ...
    return {
        ...
        topK: ragState.topK ?? 5,
        threshold: ragState.threshold ?? 0.5, // Lowered default for broader model compatibility
...
function debugLog(message, data = null) {
    // DEPRECATED: Use ragLogger.verbose() instead for consistent, controllable logging.
    // This function is kept for backward compatibility but can be removed in the future.
    ragLogger.verbose(message, data);
}
...
async function queryRAG(characterName, queryText) {
    ...
    // FALLBACK: If no collections were activated by triggers, search ALL active collections.
    // This prevents the system from returning nothing if triggers are misconfigured.
    if (activatedCollections.size === 0 && allCollectionNames.length > 0) {
        console.warn('⚠️ [RAGBooks] No collections activated by triggers. Falling back to searching ALL active collections.');
        allCollectionNames.forEach(collectionId => activatedCollections.add(collectionId));
        debugLog('Fallback activated collections:', Array.from(activatedCollections));
    }
    ...
    try {
        ...
        if (ragChunks.length > 0) {
            ...
            ragChunks.forEach((chunk, i) => {
                ...
                const source = `${chunk.collectionType || 'custom'}/${chunk.collectionName || 'Unknown'}`;
                console.log(`   ${i + 1}. [${section}] ${preview}... (score: ${score}, ${size} chars, from: ${source})`);
            });

            await injectRAGResults(characterName, ragChunks);
            console.log('💉 RAG: Injection complete');
        } else {
            console.log('✅ RAG: No relevant chunks found, skipping injection.');
            setExtensionPrompt(RAG_PROMPT_TAG, '', extension_prompt_types.IN_PROMPT, settings.injectionDepth, false, promptRole);
        }
    } catch (error) {
...
async function carrotKernelRagInterceptor(chatArray, contextSize, abort, type) {
    ...
    try {
        ...
        if (ragChunks.length > 0) {
            ...
            console.log('📦 RAG: Chunks being injected:');
            ragChunks.forEach((chunk, i) => {
                console.log(`   ${i + 1}. [${chunk.section || 'Untitled'}] ${chunk.text.substring(0, 60)}... (${chunk.text.length} chars)`);
            });
        } else {
            console.log('✅ RAG: No relevant chunks found, skipping injection.');
        }
